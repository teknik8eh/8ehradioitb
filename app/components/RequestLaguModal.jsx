...
export default function RequestLaguModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      try {
        ...
        if (data.session) {
          setHasSession(true);
          setRemaining(Math.max(0, apiLimit - (data.session.requestCount || 0)));
        } else {
          setHasSession(false);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    // Listen event dari TopLiveBar
    const handler = (e) => setOnAir(e.detail.onAir);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status === 403) {
        setSubmitError("Siaran sedang tidak live. Request tidak dapat dikirim.");
        return;
      }
      if (res.status === 429) {
      ...
      if (!res.ok) {
        setSubmitError("Gagal mengirim request. Coba lagi.");
        return;
      }
  ...
  if (!onAir) {
    return (
      <div
        className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn"
        onClick={onClose}
      >
        <div
        ...
        >
          ...
          <h2 className="text-lg font-bold text-gray-900 mb-2">Siaran Belum Live</h2>
    ...
    return (
      ...
      >
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-auto p-10 text-center">
          <svg className="w-6 h-6 animate-spin mx-auto text-red-400" viewBox="0 0 24 24" fill="none">
      ...
      >
        ...
        >
          ...
          <input
            ...
            maxLength={30}
            autoFocus
            className="w-full border-2 border-gray-200 focus:border-red-500 focus:ring-2 focus:ring-red-100 rounded-xl px-4 py-2.5 text-sm outline-none transition-colors"
          />
          ...
          <button
            ...
            className={`w-full mt-4 py-3 rounded-xl font-semibold text-sm transition-all ${
              !isCreatingSession && guestNameInput.trim().length >= 2
          ...
          <div className="px-6 pb-6 space-y-4">
            ...
            <div className="relative" ref={dropdownRef}>
              <div className={`flex items-center border-2 rounded-xl px-3 py-2 transition-colors ${
                query ? "border-red-500 ring-2 ring-red-100" : "border-gray-200 focus-within:border-red-500 focus-within:ring-2 focus-within:ring-red-100"
              }`}>
                {isSearching ? (
                  <svg className="w-4 h-4 text-red-400 animate-spin mr-2 flex-shrink-0" viewBox="0 0 24 24" fill="none">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"/>
                  </svg>
                ) : (
            ...
            {showManual && (
              <div className="space-y-2 p-3 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-red-400 focus:ring-1 focus:ring-red-100"
                />
                <input
                  type="text"
                  value={manualArtist}
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 resize-none transition-colors"
              />
            ...
            <p className="text-sm text-gray-600">
              Sisa request:{" "}
              <span className={`font-bold ${remaining === 0 ? "text-red-500" : "text-gray-900"}`}>
                {remaining}/{limit}
              </span>
            </p>
